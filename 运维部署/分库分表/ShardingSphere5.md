...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/362d22168bf344687ec0c206aa115807.jpg)
...
![image-20220804223138651](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804223138651.png)
...
- 在实际设计过程中，每个系统不可能只处理一种数据，而是包含多种类型的数据，`有的数据必须选择 CP，有的数据必须选择 AP，分布式系统理论上不可能选择 CA 架构。`
  ...
  ![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/6e7d7bd54d7a4eb67918080863d354d7.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/2ccafe41de9bd7f8dec4658f004310d6.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/71f41d46cc5c0405f4d4dc944b4350c9.jpg)

下图展示了根据业务需要，将用户表和订单表垂直分片到不同的数据库的方案：

![image-20220804221855449](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804221855449.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/136bc2f01919edcb8271df6f7e71af40.jpg)
...
![image-20220804222212087](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804222212087.png)
...
![image-20220804223321167](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804223321167.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/f8d538f9201e3ebee37dfdcd1922e9df.jpg)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/2a2dba7f07581fd055d9cd5a3aa8388e.jpg)
...
![image-20220804195402870](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804195402870.png)
...
![image-20220804195432673](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804195432673.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220714133617856.png)
...
![image-20220807183231101](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220807183231101.png)
...
```
docker run -d -p 3306:3306 -v D:\environment\docker\mysql\master\conf:/etc/mysql/conf.d -v D:\environment\docker\mysql\master\data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=Hangju@2025 --name mysql-master mysql:8.0.44
```
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/0.08703112216569037.png)
...
ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'Hangju@2025';
#如果出现ERROR 1524 (HY000): Plugin 'mysql_native_password' is not loaded
#MySQL8.4版本开始及以后官方将对mysql_native_password验证插件不再默认启用，如果我们要重新启用则需添加mysql-native-password=ON然后启动服务器 在my.cnf中
mysql-native-password=ON
#或者
ALTER USER 'root'@'%' IDENTIFIED WITH caching_sha2_password  BY 'Hangju@2025';
...
-- 设置密码
ALTER USER 'mysql_slave'@'%' IDENTIFIED WITH mysql_native_password  BY 'Hangju@2025';
-- 授予复制权限
...
![image-20220804191852164](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220804191852164.png)
...
```sql
docker run -d -p 3307:3306 -v D:\environment\docker\mysql\slave1\conf:/etc/mysql/conf.d -v D:\environment\docker\mysql\slave1\data:/var/lib/mysql -e MYSQL_ROOT_PASSWORD=Hangju@2025 --name mysql-slave1 mysql:8.0.44
```
...
```sql
CHANGE MASTER TO MASTER_HOST='192.168.2.137', 
MASTER_USER='mysql_slave',MASTER_PASSWORD='Hangju@2025', MASTER_PORT=3306,
MASTER_LOG_FILE='binlog.000003',MASTER_LOG_POS=780;
```
...
如果你确定这个 `ALTER USER` 对业务无影响（比如只是改复制用户的密码），可以 **跳过这一条 binlog**：
...
-- 跳过 1 个事件（即那条 ALTER USER）
SET GLOBAL sql_replica_skip_counter = 1;

START REPLICA;
```

> ⚠️ 注意：MySQL 8.0+ 推荐用 `sql_replica_skip_counter`（旧版是 `sql_slave_skip_counter`
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220715000533951.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220714235426120.png)
...
![img](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220715004850504.png)
...
![image-20220807232456342](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220807232456342.png)
...
docker run -d \
-p 3306:3306 \
-v D:\environment\docker\mysql/conf:/etc/mysql/conf.d \
-v D:\environment\docker\mysql/data:/var/lib/mysql \
-e MYSQL_ROOT_PASSWORD=123456 \
--name mysql-slave \
mysql:8.4
```
...
![image-20220810163534068](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220810163534068.png)
...
![image-20220808033239206](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220808033239206.png)
...
![image-20220819152009158](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220819152009158.png)
...
![image-20220819151154763](https://cdn.jsdelivr.net/gh/mayuhaos/blog-images@main/hj-images/image-20220819151154763.png)
